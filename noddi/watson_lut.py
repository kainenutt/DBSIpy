...
with concentration parameter κ (kappa):
- κ → ∞: Perfect alignment (no dispersion)
- κ → 0: Isotropic distribution
...
def watson_integral_lookup_table(
    ...
    S(b, κ) = exp(-b * d_ic) * I(b, κ)
    
    where I(b, κ) is the integral over the Watson distribution.
    
    Parameters
    ----------
    b_values : ndarray, optional
        B-values to tabulate (s/mm²). Default: 0-6000 in steps of 50
    ...
    integral_table : ndarray, shape (n_b, n_kappa)
        Watson integral I(b, κ)
    """
    if b_values is None:
        # Default: 0 to 6000 s/mm² in steps of 50
...
def _watson_integral_legendre(b: float, kappa: float, n_terms: int = 20) -> float:
    ...
    I(b, κ) = Σ_n (2n+1)/2 * P_n(0) * dawson(√(n(n+1)b/κ)) / √(n(n+1)b/κ)
...
def od_to_kappa(od: torch.Tensor) -> torch.Tensor:
    """
    Convert Orientation Dispersion Index (ODI) to Watson concentration κ.
    ...
    """
    epsilon = 1e-6
    # Avoid division by zero and tan(π/2)
...
def kappa_to_od(kappa: torch.Tensor) -> torch.Tensor:
    """
    Convert Watson concentration κ to Orientation Dispersion Index (ODI).
...
if __name__ == "__main__":
    ...
    print(f"  B-values: {b_vals.min():.0f} to {b_vals.max():.0f} s/mm² ({len(b_vals)} points)")
    ...
    for od, kappa, od_recon in zip(test_ods, kappas, ods_reconstructed):
        print(f"  OD={od:.2f} → κ={kappa:.4f} → OD={od_recon:.2f}")
    ...
    for b, k in zip(test_b, test_kappa):
        ...
        print(f"  I(b={b:.0f}, κ={k:.2f}) = {integral:.4f}")
