...
# UNITS: Diffusion in mm�/s (multiply by 1000 for �m�/ms output)
DIFFUSION_MIN = -0.005  # mm�/s (allow small negative for numerical stability)
DIFFUSION_MAX = 0.005   # mm�/s = 5 �m�/ms (max physiological diffusivity)
...
class diffusion_tensor_model():
    ...
    UNITS CONVENTION:
    - Input bvals: s/mm� (MRI standard)
    - Internal diffusion tensor: mm�/s (SI standard)
    - Output diffusivity (AD, RD, MD): �m�/ms (1 mm�/s = 1000 �m�/ms)
    """
...
class fit_diffusion_tensor_model:
    def __init__(self, predicted_signal: torch.tensor, tensor_elements: torch.tensor, device: str):
        ...
        # Dxx, Dyy, Dzz should be in [0, 0.005] mm�/s (0-5 �m�/ms)
        ...
        self.evals, self.evecs = _robust_sym_eigh_3x3_batched(
            ...
            min_eigval=0.0001,  # 0.1 �m�/ms minimum eigenvalue
            max_batch_size=None  # Auto-detect safe batch size (200K for CUDA, 1M for CPU)
        )
        
        # Apply physiological constraints: diffusivities between 0.1-3.5 �m�/ms (0.0001-0.0035 mm�/s)
...
# Kurtosis engine implementation removed.
...
class NODDIModel:
    ...
    def __init__(self, bvals: torch.Tensor, bvecs: torch.Tensor, device='cpu'):
        ...
        self.d_ic_fixed = 1.7e-3  # mm�/s = 1.7 �m�/ms (intra-cellular)
        self.d_iso_fixed = 3.0e-3  # mm�/s = 3.0 �m�/ms (free water)
    ...
    def _initialize_from_dti(self, Y: torch.Tensor) -> dict:
        ...
        try:
            ...
            fa = dti_fit.fa
            adc = dti_fit.adc * 1e-3  # Convert from �m�/ms back to mm�/s for initialization
            evecs = dti_fit.eigen_frame
            
            # Detect CSF-like voxels: low FA + high ADC
            # CSF: FA < 0.2, ADC > 2.5e-3 mm�/s (2.5 �m�/ms)
            ...
            # Typical brain: 1.0-2.0 �m�/ms (0.001-0.002 mm�/s)
...
class NODDIOptimizer(torch.nn.Module):
    ...
    def __init__(self, n_voxels, initial_params, bvals, bvecs, watson_lut,
        ...
        #   dO = df d�, where � = cos(?) ? [-1, 1], f ? [0, 2p).
        # The Watson density is ? exp(? �^2), and is normalized numerically per-voxel.
        ...
        # Range matches `src/maps/noddi_maps.py`: 0.5�3.0 �m�/ms (0.0005�0.0030 mm�/s).
        ...
        if not self.use_tortuosity:
            ...
            # Range matches `src/maps/noddi_maps.py`: 0.1�1.5 �m�/ms (0.0001�0.0015 mm�/s).
    ...
    def d_ec_par_clamped(self):
        ...
        Smoothly constrained to 0.5�3.0 �m�/ms (0.0005�0.0030 mm�/s).
        """
        span = self._d_ec_par_max - self._d_ec_par_min
    ...
    def forward(self, voxel_idx: torch.Tensor | None = None):
        ...
        # the same (g�u)^2 grid and the same Watson weights.
    ...
    def _stick_signal_dispersed(self, *, voxel_idx: torch.Tensor | None = None):
        ...
        S_ic(b, κ) = exp(-b * d_ic) * W(b * d_ic, κ)
    ...
    def _init_watson_quadrature(self, *, n_mu: int, n_phi: int) -> None:
        ...
        mu, w_mu = _np.polynomial.legendre.leggauss(int(n_mu))  # � in [-1, 1]
    ...
    def _gradients_in_local_frame(self, n: torch.Tensor) -> torch.Tensor:
        ...
        # Project each gradient into the local basis: (g�e1, g�e2, g�n)
    ...
    def _cos2_between_g_and_u(self, g_local: torch.Tensor) -> torch.Tensor:
        """Compute (g�u)^2 for all local quadrature orientations u."""
...
class FitNODDIModel:
    ...
    @auto_attr
    def d_ic(self):
        """Intra-cellular diffusivity (�m�/ms)"""
        return self.model_params['d_ic'] * 1e3
    
    @auto_attr
    def d_ec_par(self):
        """Extra-cellular parallel diffusivity (�m�/ms)"""
        return self.model_params['d_ec_par'] * 1e3
    
    @auto_attr
    def d_ec_perp(self):
        """Extra-cellular perpendicular diffusivity (�m�/ms)"""
        return self.model_params['d_ec_perp'] * 1e3
    
    @auto_attr
    def d_iso(self):
        """Isotropic diffusivity (�m�/ms)"""
